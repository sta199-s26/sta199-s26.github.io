...
title: Lab 7
subtitle: "#canceled"
description: "Due: End of lab on Thu, April 2"
categories: Lab
filters:
  - ../remove-fmt-skip.lua
draft: false
...
In this lab, you will clean and prepare a dataset, then use visualization and logistic regression to explore patterns in hotel reservation cancellations.
...
-   `deposit_type`: ndication on if the customer made a deposit to guarantee the booking. This variable can assume three categories:
...
The dataset, called `hotel-reservations.csv`, can be found in the `data` folder.

```{r}
#| include: false
#| eval: true
hotels <- read_csv(here::here("lab", "data/hotel-reservations.csv"))
```

```{r}
hotels <- read_csv("data/hotel-reservations.csv")
```
